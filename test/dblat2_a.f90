...
!     .. Local Scalars ..
      ...
      INTEGER            I, ISNUM, J, N, NALF, NBET, NIDIM, NINC, NKB, NOUT, NTRA
...
!     .. Executable Statements ..
      ...
      open(unit=nin,file='test/dblat2.in',delim='apostrophe')
!
...
!     the result computed by DMVCH.
      TRANS = 'N'
      CALL DMVCH( TRANS, N, N, ONE, A, NMAX, X, 1, ZERO, Y, 1, YT, G, YY, EPS, ERR, FATAL, NOUT, .TRUE. )
      SAME = LDE( YY, YT, N )
...
!
 ...
 9993 FORMAT( ' TESTS OF THE DOUBLE PRECISION LEVEL 2 BLAS', //' THE F', 'OLLOWING PARAMETER VALUES WILL BE USED:' )
...
!
               TRANSL = ZERO
               CALL DMAKE( SNAME( 2: 3 ), ' ', ' ', M, N, A, NMAX, AA, LDA, KL, KU, RESET, TRANSL )
!
               DO 90 IC = 1, 3
                  TRANS = ICH( IC: IC )
...
!
                     TRANSL = HALF
                     CALL DMAKE( 'GE', ' ', ' ', 1, NL, X, 1, XX, ABS( INCX ), 0, NL - 1, RESET, TRANSL )
                     IF( NL.GT.1 )THEN
                        X( NL/2 ) = ZERO
...
!
                              TRANSL = ZERO
                              CALL DMAKE( 'GE', ' ', ' ', 1, ML, Y, 1, YY, ABS( INCY ), 0, ML - 1, RESET, TRANSL )
!
                              NC = NC + 1
!
...
!
                              ...
                              ELSE IF( BANDED )THEN
                                 ...
                                 IF( REWI ) REWIND NTRA
                                 CALL DGBMV( TRANS, M, N, KL, KU, ALPHA, AA, LDA, XX, INCX, BETA, YY, INCY )
                              END IF
!
...
!
               TRANSL = ZERO
               CALL DMAKE( SNAME( 2: 3 ), UPLO, ' ', N, N, A, NMAX, AA, LDA, K, K, RESET, TRANSL )
!
               DO 80 IX = 1, NINC
                  INCX = INC( IX )
                  LX = ABS( INCX )*N
...
!
                  TRANSL = HALF
                  CALL DMAKE( 'GE', ' ', ' ', 1, N, X, 1, XX, ABS( INCX ), 0, N - 1, RESET, TRANSL )
                  IF( N.GT.1 )THEN
                     X( N/2 ) = ZERO
...
!
                           TRANSL = ZERO
                           CALL DMAKE( 'GE', ' ', ' ', 1, N, Y, 1, YY, ABS( INCY ), 0, N - 1, RESET, TRANSL )
!
                           NC = NC + 1
!
!                          Save every datum before calling the
...
!                             Check the result.
!
                              CALL DMVCH( 'N',N,N,ALPHA,A,NMAX,X,INCX,BETA,Y,INCY,YT,G,YY,EPS,ERR,FATAL,NOUT,.TRUE. )
                              ERRMAX = MAX( ERRMAX, ERR )
!                             If got really bad answer, report and
!                             return.
                              IF( FATAL ) GO TO 120
...
!
                     TRANSL = ZERO
                     CALL DMAKE( SNAME( 2: 3 ), UPLO, DIAG, N, N, A, NMAX, AA, LDA, K, K, RESET, TRANSL )
!
                     DO 60 IX = 1, NINC
                        INCX = INC( IX )
                        LX = ABS( INCX )*N
...
!                             Check the result.
!
                              CALL DMVCH( TRANS,N,N,ONE,A,NMAX,X,INCX,ZERO,Z,INCX,XT,G,XX,EPS,ERR,FATAL,NOUT,.TRUE. )
                           ELSE IF( SNAME( 4: 5 ).EQ.'SV' )THEN
!
...
!
   ...
   50                         CONTINUE
                              CALL DMVCH( TRANS,N,N,ONE,A,NMAX,Z,INCX,ZERO,X,INCX,XT,G,XX,EPS,ERR,FATAL,NOUT,.FALSE. )
                           END IF
                           ERRMAX = MAX( ERRMAX, ERR )
...
!
               TRANSL = HALF
               CALL DMAKE( 'GE', ' ', ' ', 1, M, X, 1, XX, ABS( INCX ), 0, M - 1, RESET, TRANSL )
               IF( M.GT.1 )THEN
                  X( M/2 ) = ZERO
...
!
                     TRANSL = ZERO
                     CALL DMAKE( SNAME( 2: 3 ), ' ', ' ', M, N, A, NMAX, AA, LDA, M - 1, N - 1, RESET, TRANSL )
!
                     NC = NC + 1
!
!                    Save every datum before calling the subroutine.
...
!
                  TRANSL = ZERO
                  CALL DMAKE( SNAME( 2: 3 ), UPLO, ' ', N, N, A, NMAX, AA, LDA, N - 1, N - 1, RESET, TRANSL )
!
                  NC = NC + 1
!
!                 Save every datum before calling the subroutine.
      ...
      DO 20 J = 1, N
         DO 10 I = 1, M
            IF( GEN.OR.( UPPER.AND.I.LE.J ).OR.( LOWER.AND.I.GE.J ) ) THEN
               IF( ( I.LE.J.AND.J - I.LE.KU ).OR.  ( I.GE.J.AND.I - J.LE.KL ) )THEN
...
!
   70 CONTINUE
      RETURN
!
 9999 FORMAT( ' ******* FATAL ERROR - COMPUTED RESULT IS LESS THAN HALF ACCURATE *******', /'           EXPECTED RESULT   COMPU', &
     &      'TED RESULT' )
