...
!     .. Executable Statements ..
      ...
      open(unit=nin,file='test/cblat3.in',delim='apostrophe')
!
!     Read name and unit number for summary output file and open file.
!
      READ( NIN, FMT = * )SUMMRY
      READ( NIN, FMT = * )NOUT
      if(SUMMRY.ne.'') OPEN( NOUT, FILE = SUMMRY )
      NOUTC = NOUT
!
...
!
                           ...
                           IF( REWI ) REWIND NTRA
                           CALL CGEMM( TRANSA, TRANSB, M, N, K, ALPHA, AA, LDA, BB, LDB, BETA, CC, LDC )
!
                           ...
                           DO 70 J = 1, N
                              ...
                              IF( TRAN )THEN
                                 DO 50 I = 1, K
                                    ...
                                    END IF
   50                            CONTINUE
                                 CALL CMMCH( TRANST, 'N', LJ, 1, 2*K, ONE, AB( JJAB ), 2*NMAX, W, 2*NMAX, BETA, C( JJ, J ), &
     &                                       NMAX, CT, G, CC( JC ), LDC, &
     &                                       EPS, ERR, FATAL, NOUT, &
     &                                       .TRUE. )
                              ELSE
