...
!  =====================================================================
      ...
      use M_blas, only : cgbmv, cgemv, cgerc, cgeru, chbmv, chemv, cher, cher2
      use M_blas, only : chpmv, chpr, chpr2, ctbmv, ctbsv, ctpmv, ctpsv, ctrmv
      use M_blas, only : ctrsv
!
...
!
  240 CONTINUE
      ...
      STOP
!
 9999 FORMAT( ' ROUTINES PASS COMPUTATIONAL TESTS IF TEST RATIO IS LESS THAN', F8.2 )
 ...
 9986 FORMAT( ' SUBPROGRAM NAME ', A6, ' NOT RECOGNIZED', /' ******* TESTS ABANDONED *******' )
...
!
               TRANSL = ZERO
               CALL CMAKE( SNAME( 2: 3 ), ' ', ' ', M, N, A, NMAX, AA, LDA, KL, KU, RESET, TRANSL )
!
               DO 90 IC = 1, 3
                  TRANS = ICH( IC: IC )
...
!
                              ...
                              ELSE IF( BANDED )THEN
                                 ...
                                 IF( REWI ) REWIND NTRA
                                 CALL CGBMV( TRANS, M, N, KL, KU, ALPHA, AA, LDA, XX, INCX, BETA, YY, INCY )
                              END IF
!
...
!
 ...
 9997 FORMAT( ' ', A6, ' COMPLETED THE COMPUTATIONAL TESTS (', I6, ' CALLS)', /' ******* BUT WITH MAXIMUM TEST RATIO', F8.2, &
     &      ' - SUSPECT *******' )
...
!
               TRANSL = ZERO
               CALL CMAKE( SNAME( 2: 3 ), UPLO, ' ', N, N, A, NMAX, AA, LDA, K, K, RESET, TRANSL )
!
               DO 80 IX = 1, NINC
                  INCX = INC( IX )
                  LX = ABS( INCX )*N
...
!
                     TRANSL = ZERO
                     CALL CMAKE( SNAME( 2: 3 ), UPLO, DIAG, N, N, A, NMAX, AA, LDA, K, K, RESET, TRANSL )
!
                     DO 60 IX = 1, NINC
                        INCX = INC( IX )
                        LX = ABS( INCX )*N
...
!
 ...
 9998 FORMAT( ' ******* FATAL ERROR - PARAMETER NUMBER ', I2, ' WAS CHANGED INCORRECTLY *******' )
 9997 FORMAT( ' ', A6, ' COMPLETED THE COMPUTATIONAL TESTS (', I6, ' C', &
     &      'ALLS)', /' ******* BUT WITH MAXIMUM TEST RATIO', F8.2, ' - SUSPECT *******' )
 9996 FORMAT( ' ******* ', A6, ' FAILED ON CALL NUMBER:' )
...
!
                     TRANSL = ZERO
                     CALL CMAKE( SNAME( 2: 3 ), ' ', ' ', M, N, A, NMAX, AA, LDA, M - 1, N - 1, RESET, TRANSL )
!
                     NC = NC + 1
!
!                    Save every datum before calling the subroutine.
...
!
   ...
   60                      CONTINUE
                        ...
                        DO 70 J = 1, N
                           ...
                           CALL CMVCH( 'N',M,1,ALPHA,Z,NMAX,W,1,ONE,A(1,J),1,YT,G,AA(1+(J-1)*LDA),EPS,ERR,FATAL,NOUT,.TRUE. )
                           ERRMAX = MAX( ERRMAX, ERR )
!                          If got really bad answer, report and return.
                           IF( FATAL ) GO TO 130
   70                   CONTINUE
...
!
                  TRANSL = ZERO
                  CALL CMAKE( SNAME( 2: 3 ), UPLO, ' ', N, N, A, NMAX, AA, LDA, N - 1, N - 1, RESET, TRANSL )
!
                  NC = NC + 1
!
!                 Save every datum before calling the subroutine.
...
!
   ...
   50                   CONTINUE
     ...
     &                              1, ONE, A( JJ, J ), 1, YT, G, AA( JA ), EPS, ERR, FATAL, NOUT, .TRUE. )
                        IF( FULL )THEN
                           IF( UPPER )THEN
                              JA = JA + LDA
                           ELSE
...
!
   80 CONTINUE
      RETURN
!
 9999 FORMAT( ' ******* FATAL ERROR - COMPUTED RESULT IS LESS THAN HALF ACCURATE *******', /'                       EXPECTED RE', &
